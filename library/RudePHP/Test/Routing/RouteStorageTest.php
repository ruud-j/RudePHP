...
class RouteStorageTest extends PHPUnit_Framework_TestCase
{
    public function testFind()
    {
        ...
        $route = new Route('GET', new RoutePath('/foo/bar'), function() {return 1;});
        $routeStorage->register($route);
        $route = new Route('GET', new RoutePath('/foo/{var}'), function() {return 2;});
        $routeStorage->register($route);
        $route = new Route('GET', new RoutePath('/foo'), function() {return 3;});
        $routeStorage->register($route);
        $route = new Route('GET', new RoutePath('/foo/not_reachable'), function() {return 4;});
        $routeStorage->register($route);
        $route = new Route('GET', new RoutePath('/'), function() {return 5;});
        $routeStorage->register($route);
